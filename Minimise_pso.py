...
for i in range(dataset.shape[0]):
    ...
    result = minimize(lambda x, row: np.sum(row ** x[0]), x0, dataset[i],constraints, options = {'g_rate': 1., 'l_rate': 1., 'max_velocity': 4.,'stable_iter': 50,'sttol': 1e-4,'savefile': 'data_new_results.csv'})
